...
if(ok) ok = get_obj_class(self, dname, dset_id, get_class)

call H5Dclose_f(dset_id, ier)
call estop(ier, "get_class:H5Dclose", self%filename, dname, ok=ok)

if(.not.ok) error stop "ERROR:h5fortran:get_class: failed to get class"
...
call estop(ier, "get_obj_class:H5[A,D]get_type", self%filename, obj_name, ok=ok)
endif

if (ok) then
call H5Tget_native_type_f(obj_dtype, H5T_DIR_ASCEND_F, native_dtype, ier)
call estop(ier, "get_obj_class:H5Tget_native_type", self%filename, obj_name, ok=ok)
endif

!> compose datatype inferred
if(ok) then
call H5Tget_class_f(native_dtype, class, ier)
call estop(ier, "get_obj_class:H5Tget_class", self%filename, obj_name, ok=ok)
endif

if(ok .and. present(size_bytes)) then
  call H5Tget_size_f(native_dtype, size_bytes, ier)
  call estop(ier, "get_obj_class:H5Tget_size", self%filename, obj_name, ok=ok)
endif

if (ok) then
call H5Tclose_f(native_dtype, ier)
call estop(ier, "get_obj_class:H5Tclose", self%filename, obj_name, ok=ok)
endif

if(ok .and. present(pad_type)) then
  ...
  call estop(ier, "get_obj_class:H5Tget_strpad", self%filename, obj_name, ok=ok)
endif
